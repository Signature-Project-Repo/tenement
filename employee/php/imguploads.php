<?php
	...
	$user=mysqli_fetch_assoc(select_data("select img from employee_register where email='$email';"));
	...
	if (move_uploaded_file($_FILES["pro"]["tmp_name"], $target_file)) {
		$sql=update_data("update employee_register set img='$Filename' where email='$email'");
		echo 1;
		exit();
	} 
	else {
		$sql=update_data("update employee_register set img='default.png' where email='$email'");
		echo 0;
		exit();
	}
?>